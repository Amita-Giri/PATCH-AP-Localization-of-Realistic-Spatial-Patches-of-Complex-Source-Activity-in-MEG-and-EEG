...
def generator(fwd, use_cov=True, batch_size=1284, batch_repetitions=30, n_sources=10, 
    ...
    while True:
        ...
        x = np.stack([add_white_noise(xx, snr_level, rng, correlation_mode=correlation_mode, noise_color_coeff=noise_color_coeff) for (xx, snr_level) in zip(x, snr_levels)], axis=0)
...
def add_white_noise(X_clean, snr, rng, noise_color_coeff=0.5, correlation_mode=None):
    ...
    if correlation_mode == "cholesky":
        ...
        pass
    elif type(correlation_mode) == np.ndarray:
        coeff_mat = correlation_mode.copy()
        # This should be done in advance to keep the code fast
        # if not np.all(np.linalg.eigvals(coeff_mat) > 0):
        #     coeff_mat += 1e-1 * np.eye(len(coeff_mat))

        # X_noise = np.linalg.cholesky( coeff_mat ) @ X_noise
        X_noise = coeff_mat @ X_noise
